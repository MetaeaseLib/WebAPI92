...
A template repository for a Cloud Run webapi, written in Java.
...
1. Build the container:
    ```bash
    mvn clean compile jib:build -Dimage=$REGION-docker.pkg.dev/$GOOGLE_CLOUD_PROJECT/$REPOSITORY/metaease-webapi
    ```

1. Deploy to Cloud Run:
    ```bash
    gcloud run deploy metaease-webapi \
      --image $REGION-docker.pkg.dev/$GOOGLE_CLOUD_PROJECT/$REPOSITORY/metaease-webapi \
      --region $REGION
    ```
...
2. Run system tests
    ...
    gcloud builds submit \
        ...
        --substitutions 'COMMIT_SHA=manual,REPO_NAME=cloud-run-metaease-webapi-java'
    ```
